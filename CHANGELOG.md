...
- **Three new curriculum lessons teaching the above**: *WORD Motion (W)*, *WORD End (E)* (both after Word Movement, contrasting `w`/`W` and `e`/`E` — issue [#3](https://github.com/renzorlive/vimmaster/issues/3)), and *First Non-Blank (^)* (after Line Bounds). The curriculum grows from 18 to 21 core lessons; each ships a `solution` the Golden Suite replays to prove it's winnable, and each passes the L013 rule (not satisfiable by its initial state).
